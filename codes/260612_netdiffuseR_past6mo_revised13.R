...
for (sch in all_schools) {
  ...
  for (wi in seq_along(sch_waves)) {
    ...
    susc_f_raw <- 5 - susc_f_raw  # 1→4, 2→3, 3→2, 4→1: higher = more susceptible
    susc_y_raw <- 5 - susc_y_raw
...
# Demographics | Network at GP3/GP8 | sm_ecig_any added per Tom's request
# Exposure rows excluded ONLY when they directly define the subgroup (definitional overlap)
...
# Col4 Low Threshold: no definitional overlap → keep both
or_col1 <- run_subgroup_or(full_df, "col1_no_exposure", "is_user",
                           "Col1: Users-NoExposure",
                           exclude_covs = c("expo_lag","expo_gp8"),
                           use_toa_fe = TRUE)
...
or_col_both0 <- run_subgroup_or(full_df, "col_both0", "is_user",
                           ...
                           exclude_covs = c("expo_lag","expo_gp8","friends_ecig_toa"),
                           use_toa_fe = TRUE)
...
or_col4 <- run_subgroup_or(full_df, "col4_exposure",  "is_nonuser",
                           ...
                           exclude_covs = c("friends_ecig_toa","expo_gp8"))
or_col5 <- run_subgroup_or(full_df, "col5_friends",   "is_nonuser",
                           ...
                           exclude_covs = c("friends_ecig_gp3","friends_ecig_gp8",
                                            "friends_ecig_toa","expo_lag","expo_gp8"))
...
if (!is.null(sm_table) && nrow(sm_table) > 0) {
  ...
  print(sm_table)
  write.csv(sm_table,
            file.path(data_path, "260609_netdiffuseR_past6mo_social_media_sensitivity.csv"),
            row.names=FALSE)
