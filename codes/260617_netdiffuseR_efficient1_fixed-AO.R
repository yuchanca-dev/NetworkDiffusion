# =============================================================================
# 260617_netdiffuseR_efficient1_fixed-AO.R
# Built on 260617_netdiffuseR_efficient1-AO.R (same efficient slim-cache loader),
# adding TWO statistical fixes (it does NOT touch the mixed adoption model — that
# is the separate efficient2 line):
#   * Problem 1 — threshold complete separation: the grouped-binomial threshold
#     model is now fit by Firth (brglm2 `method="brglmFit"`) so the GP3 / School-105
#     separations resolve and the `-20.63 / 17.x` block becomes finite. (≈ run_threshold_reg)
#   * Problem 2 — par_edu "Don't know" (9) leak + bad W7+ remap: par_edu is recoded
#     to a homogeneous 1..6 scale with 9 -> NA at all waves. (≈ remap_par_edu)
# Everything else (incl. the efficient data loader) is identical to efficient1.
#
...
run_exposure_reg <- function(reg_data) {
  ...
  ever_adopted <- unique(reg_final$id[reg_final$adopt_next == 1])
  n_students   <- length(unique(reg_final$id))
  n_adopt      <- length(ever_adopted)
  n_nonadopt   <- n_students - n_adopt
  cat(sprintf("Unique students: %d (adopters: %d, non-adopters: %d)\n",
              n_students, n_adopt, n_nonadopt))
  # glmer with person-level random intercept — same student has multiple rows (person-period)
  fml <- as.formula(paste("adopt_next ~ exposure + factor(grade_period) +",
                           paste(exp_covs, collapse = " + "),
                           "+ (1 | id) + (1 | school)"))
  cat("Formula:", deparse(fml), "\n")
  fit <- lme4::glmer(fml, data = reg_final, family = binomial,
  ...
  print(out)
  or_exp <- exp(lme4::fixef(fit)["exposure"])
  ci_exp <- exp(confint(fit, "exposure", method = "Wald"))
