...
# Plots
pdf(file.path(data_path, "0411_netdiffuseR_adopters.pdf"), width = 9, height = 5)
...
pdf(file.path(data_path, "0411_netdiffuseR_hazard.pdf"), width = 9, height = 5)
...
pdf(file.path(data_path, "0411_netdiffuseR_threshold_boxplot.pdf"), width = 9, height = 5)
par(mar = c(5, 4, 4, 2))
...
pdf(file.path(data_path, "0411_netdiffuseR_byschool.pdf"), width = 11, height = 6)
par(mar = c(5, 4, 4, 10))
...
if (!is.null(fit_thr)) {
  ...
  write.csv(thr_result,
            file.path(data_path, "0411_netdiffuseR_threshold_regression.csv"),
            row.names = FALSE)
...
write.csv(adopters,
          file.path(data_path, "0411_netdiffuseR_node_outcomes_adopters.csv"),
          row.names = FALSE)
...
write.csv(exp_result,
          file.path(data_path, "0411_netdiffuseR_exposure_regression.csv"),
          row.names = FALSE)
...
write.csv(cls_df,
          file.path(data_path, "0411_netdiffuseR_classification.csv"),
          row.names = FALSE)
cat("Saved: classification.csv\n")

# Per-school summary
write.csv(results_table,
          file.path(data_path, "0411_netdiffuseR_school_summary.csv"),
          row.names = FALSE)
...
sheets <- list(
  school_summary       = "0411_netdiffuseR_school_summary.csv",
  exposure_regression  = "0411_netdiffuseR_exposure_regression.csv",
  threshold_regression = "0411_netdiffuseR_threshold_regression.csv",
  classification       = "0411_netdiffuseR_classification.csv",
  node_outcomes        = "0411_netdiffuseR_node_outcomes_adopters.csv"
)
...
saveWorkbook(wb,
             file.path(data_path, "0411_netdiffuseR_ALL_RESULTS.xlsx"),
             overwrite = TRUE)
...
plot_dir <- file.path(data_path, "0411_netdiffuseR_network_plots")
...
for (sch_key in names(diffnet_list)) {
  ...
  out_file <- file.path(plot_dir,
                         sprintf("0411_diffnet_school%s.pdf", sch_key))
...
pdf(file.path(data_path, "0411_netdiffuseR_diffnet_representative.pdf"),
    width = 14, height = 10)
...
save(diffnet_list, diffnet_all, results_table, node_df,
     file = file.path(data_path, "0411_netdiffuseR_everuse.RData"))
...
cat("Excel : 0411_netdiffuseR_ALL_RESULTS.xlsx\n")
cat("RData : 0411_netdiffuseR_everuse.RData\n")
