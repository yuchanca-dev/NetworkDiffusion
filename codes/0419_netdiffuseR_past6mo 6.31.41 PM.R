...
# Plots
pdf(file.path(data_path, "0419_netdiffuseR_past6mo_adopters.pdf"), width = 9, height = 5)
...
pdf(file.path(data_path, "0419_netdiffuseR_past6mo_hazard.pdf"), width = 9, height = 5)
...
pdf(file.path(data_path, "0419_netdiffuseR_past6mo_threshold_boxplot.pdf"), width = 9, height = 5)
par(mar = c(5, 4, 4, 2))
...
pdf(file.path(data_path, "0419_netdiffuseR_past6mo_byschool.pdf"), width = 11, height = 6)
par(mar = c(5, 4, 4, 10))
...
fit_thr <- tryCatch(
  glm(fml_thr, data = thr_sub[complete.cases(thr_sub[, c("threshold","cohort","female","hispanic","asian")]),],
...
if (!is.null(fit_thr)) {
  ...
  write.csv(thr_result,
            file.path(data_path, "0419_netdiffuseR_past6mo_threshold_regression.csv"),
            row.names = FALSE)
...
write.csv(adopters,
          file.path(data_path, "0419_netdiffuseR_past6mo_node_outcomes_adopters.csv"),
          row.names = FALSE)
...
for (gi in 1:n_period) {
  ...
  if (gi < n_period) {
    adopt_next_vals <- as.integer(adopt_mat[not_yet, gi + 1])
  } else {
    adopt_next_vals <- rep(0L, length(not_yet))
  }
  reg_rows[[gi]] <- data.frame(
    id           = vertex_ids_clean[not_yet],
    adopt_next   = adopt_next_vals,
...
exp_covs <- c("cohort", "female", "hispanic", "asian")
...
req_vars <- c("adopt_next", "exposure", "grade_period", "cohort", "female", "hispanic", "asian")
...
write.csv(exp_result,
          file.path(data_path, "0419_netdiffuseR_past6mo_exposure_regression.csv"),
          row.names = FALSE)
...
write.csv(cls_df,
          file.path(data_path, "0419_netdiffuseR_past6mo_classification.csv"),
          row.names = FALSE)
cat("Saved: classification.csv\n")

# Per-school summary
write.csv(results_table,
          file.path(data_path, "0419_netdiffuseR_past6mo_school_summary.csv"),
          row.names = FALSE)
...
sheets <- list(
  school_summary       = "0419_netdiffuseR_past6mo_school_summary.csv",
  exposure_regression  = "0419_netdiffuseR_past6mo_exposure_regression.csv",
  threshold_regression = "0419_netdiffuseR_past6mo_threshold_regression.csv",
  classification       = "0419_netdiffuseR_past6mo_classification.csv",
  node_outcomes        = "0419_netdiffuseR_past6mo_node_outcomes_adopters.csv"
)
...
saveWorkbook(wb,
             file.path(data_path, "0419_netdiffuseR_past6mo_ALL_RESULTS.xlsx"),
             overwrite = TRUE)
...
plot_dir <- file.path(data_path, "0419_netdiffuseR_past6mo_network_plots")
...
for (sch_key in names(diffnet_list)) {
  ...
  out_file <- file.path(plot_dir,
                         sprintf("0419_diffnet_past6mo_school%s.pdf", sch_key))
...
pdf(file.path(data_path, "0419_netdiffuseR_past6mo_diffnet_representative.pdf"),
    width = 14, height = 10)
...
save(diffnet_list, diffnet_all, results_table, node_df,
     file = file.path(data_path, "0419_netdiffuseR_past6mo_everuse.RData"))
...
cat("Excel : 0419_netdiffuseR_past6mo_ALL_RESULTS.xlsx\n")
cat("RData : 0419_netdiffuseR_past6mo_everuse.RData\n")
